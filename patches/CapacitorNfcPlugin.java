...
public class CapacitorNfcPlugin extends Plugin {
    ...
    private void writeMifarePages(MifareUltralight mifare, byte[] data, int startPage) throws IOException {
        int pageSize = 4;
        int totalPages = (data.length + pageSize - 1) / pageSize;
        for (int i = 0; i < totalPages; i++) {
            int offset = i * pageSize;
            int len = Math.min(pageSize, data.length - offset);
            byte[] page = new byte[pageSize];
            System.arraycopy(data, offset, page, 0, len);
            mifare.writePage(startPage + i, page);
        }
    }
